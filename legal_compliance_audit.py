...
class LegalComplianceAudit:
    ...
    def get_page_content(self, url):
        ...
        try:
            ...
            self.log_issue('CRITICAL', 'Accessibility', f"Failed to fetch {url}: {str(e)}")
            return None
    ...
    def check_html_structure(self, soup, page_name):
        ...
        if h1_count != 1:
            self.log_issue('WARNING', 'SEO', f"{page_name}: Found {h1_count} H1 tags, should be exactly 1")
    ...
    def check_legal_content_completeness(self, soup, page_type):
        ...
        for section_name, pattern in required_sections:
            if not re.search(pattern, text_content):
                self.log_issue('WARNING', 'Completeness', 
    ...
    def check_accessibility_compliance(self, soup, page_name):
        ...
        for img in images:
            if not img.get('alt'):
                self.log_issue('CRITICAL', 'Accessibility', 
        ...
        for link in links:
            ...
            if not link_text or link_text.lower() in ['click here', 'here', 'read more']:
                self.log_issue('WARNING', 'Accessibility', 
        ...
        if not html_tag or not html_tag.get('lang'):
            self.log_issue('WARNING', 'Accessibility', f"{page_name}: Missing lang attribute on html tag")
    ...
    def check_legal_language_quality(self, soup, page_type):
        ...
        if vague_count > 10:
            self.log_issue('RECOMMENDATION', 'Legal Quality', 
        ...
        if not re.search(email_pattern, text_content):
            self.log_issue('WARNING', 'Legal Quality', 
    ...
    def check_gdpr_compliance(self, soup, page_type):
        ...
        for requirement, pattern in gdpr_requirements:
            if not re.search(pattern, text_content):
                self.log_issue('RECOMMENDATION', 'GDPR', 
    ...
    def check_css_and_styling(self, soup, page_name):
        ...
        if len(inline_styles) > 5:
            self.log_issue('RECOMMENDATION', 'Code Quality', 
                         f"{page_name}: High use of inline styles ({len(inline_styles)}). Consider CSS classes.")
        
        # Check for responsive design indicators
        meta_viewport = soup.find('meta', attrs={'name': 'viewport'})
        if not meta_viewport:
            self.log_issue('WARNING', 'Responsive Design', f"{page_name}: Missing viewport meta tag")
    ...
    def check_seo_optimization(self, soup, page_name):
        ...
        if not meta_desc:
            ...
            if desc_length < 120 or desc_length > 160:
                self.log_issue('RECOMMENDATION', 'SEO', 
    ...
    def check_performance_indicators(self, soup, page_name):
        ...
        if len(external_css) > 3:
            self.log_issue('RECOMMENDATION', 'Performance', 
        ...
        blocking_scripts = [s for s in external_scripts if not s.get('async') and not s.get('defer')]
    ...
    def generate_report(self):
        ...
        if total_items == 0:
            ...
            weighted_score = max(0, 100 - (len(self.issues) * 10) - (len(self.warnings) * 5) - (len(self.recommendations) * 2))

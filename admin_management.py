...
def list_admins():
    ...
    with app.app_context():
        ...
        for admin in admins:
            ...
            print(
                f"Last Login: {admin.last_login.strftime('%Y-%m-%d %H:%M:%S') if admin.last_login else 'Never'}"
            )
    ...
    with app.app_context():
        ...
        for admin in admins:
            ...
            if password_age.days > 90:
                issues.append(
                    f"⚠️  {admin.username}: Password is {password_age.days} days old (recommend change)"
                )

            # Check failed login attempts
            if admin.login_attempts > 0:
                issues.append(
                    f"⚠️  {admin.username}: {admin.login_attempts} recent failed login attempts"
                )

...
class Project(db.Model):
    ...
    intake_submission_id = db.Column(
        db.Integer, db.ForeignKey("intake_submission.id"), nullable=True
    )
    ...
    timeline_events = db.relationship(
        "ProjectTimelineEvent", backref="project", lazy="dynamic"
    )
...
class ProjectTimelineEvent(db.Model):
    ...
    )  # status_change, milestone, note, file_upload, client_feedback
    title = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text, nullable=True)
    event_date = db.Column(db.DateTime, default=datetime.utcnow)
    created_by = db.Column(
        db.String(100), nullable=True
    )  # admin user who created the event

...
class WCAGAudit:
    def __init__(self, base_url="http://localhost:5000"):
        ...
        self.results = {"summary": {}, "issues": [], "passed": [], "warnings": []}
        ...
        for test in color_tests:
            ...
            if ratio >= required_ratio:
                self.results["passed"].append(
                    f"✓ {test['element']}: {ratio:.2f}:1 (passes {test['level']})"
                )
            else:
                issues.append(
                    f"✗ {test['element']}: {ratio:.2f}:1 (fails {test['level']}, needs {required_ratio}:1)"
                )
    ...
    def check_headings_structure(self, soup):
        ...
        for heading in headings:
            ...
            if level > prev_level + 1:
                issues.append(
                    f"✗ Heading sequence skipped: {heading.name} after h{prev_level}"
                )
            prev_level = level
    ...
    def check_images(self, soup):
        ...
        for img in images:
            ...
            if alt is None:
                ...
                self.results["passed"].append(
                    f"✓ Decorative image with empty alt: {src}"
                )
            else:
                self.results["passed"].append(
                    f"✓ Image has descriptive alt text: {src}"
                )
    ...
    def check_links(self, soup):
        ...
        for link in links:
            ...
            if not text and not aria_label and not title:
                ...
                self.results["passed"].append(
                    f"✓ Link has descriptive text: '{text or aria_label}'"
                )
            ...
            for input_elem in inputs:
                ...
                if input_id:
                    ...
                    if not label:
                        ...
                        self.results["passed"].append(
                            f"✓ Input has proper label: {input_id}"
                        )
                else:
    ...
    def check_buttons(self, soup):
        ...
        for button in buttons:
            ...
            if not text and not aria_label and not title:
                ...
                self.results["passed"].append(
                    f"✓ Button has accessible text: '{text or aria_label}'"
                )
    ...
    def check_aria_attributes(self, soup):
        ...
        for elem in interactive_elements:
            if elem.get("aria-label"):
                self.results["passed"].append(
                    f"✓ Interactive element has aria-label: {elem.name}"
                )
    ...
    def check_page_title(self, soup):
        ...
        if not title:
            ...
            self.results["passed"].append(
                f"✓ Page has descriptive title: '{title.get_text(strip=True)}'"
            )
    ...
    def check_language(self, soup):
        ...
        if not html:
            ...
            self.results["passed"].append(
                f"✓ Page language declared: {html.get('lang')}"
            )

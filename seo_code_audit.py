...
class SEOCodeAudit:
    ...
    def check_seo_fundamentals(self, soup, page_name, url):
        ...
        if not title:
            ...
            if not title_text:
                ...
                self.log_issue(
                    "HIGH",
                    "SEO",
                    f"{page_name}: Title too long ({len(title_text)} chars), should be ≤60",
                )
            elif len(title_text) < 30:
                self.log_issue(
                    "RECOMMENDATION",
                    "SEO",
                    f"{page_name}: Title quite short ({len(title_text)} chars), consider expanding",
                )
            else:
                self.passed_checks.append(
                    {
                        "message": f"{page_name}: Title length optimal ({len(title_text)} chars)"
                    }
                )
        ...
        if not meta_desc:
            ...
            if not desc_content:
                self.log_issue(
                    "CRITICAL", "SEO", f"{page_name}: Empty meta description"
                )
            elif len(desc_content) > 160:
                self.log_issue(
                    "HIGH",
                    "SEO",
                    f"{page_name}: Meta description too long ({len(desc_content)} chars), should be ≤160",
                )
            elif len(desc_content) < 120:
                self.log_issue(
                    "RECOMMENDATION",
                    "SEO",
                    f"{page_name}: Meta description short ({len(desc_content)} chars), consider expanding to 120-160",
                )
            else:
                self.passed_checks.append(
                    {
                        "message": f"{page_name}: Meta description length optimal ({len(desc_content)} chars)"
                    }
                )
        ...
        if not h1_tags:
            ...
            self.log_issue(
                "HIGH",
                "SEO",
                f"{page_name}: Multiple H1 tags ({len(h1_tags)}) found, should have exactly one",
            )
        else:
            ...
            if not h1_text:
                ...
                self.passed_checks.append(
                    {"message": f"{page_name}: H1 tag properly implemented"}
                )
            ...
            for i in range(1, len(heading_levels)):
                if heading_levels[i] - heading_levels[i - 1] > 1:
                    self.log_issue(
                        "RECOMMENDATION",
                        "SEO",
                        f"{page_name}: Heading hierarchy skip detected (H{heading_levels[i-1]} to H{heading_levels[i]})",
                    )
                    break
        ...
        if not canonical:
            self.log_issue(
                "RECOMMENDATION", "SEO", f"{page_name}: Missing canonical URL"
            )
        ...
        if not html_tag or not html_tag.get("lang"):
            self.log_issue(
                "HIGH", "SEO", f"{page_name}: Missing lang attribute on html tag"
            )
        else:
            self.passed_checks.append(
                {"message": f"{page_name}: Lang attribute properly set"}
            )
    ...
    def check_performance_seo(self, soup, page_name):
        ...
        for img in images:
            if not img.get("alt"):
                ...
                self.log_issue(
                    "HIGH",
                    "SEO",
                    f"{page_name}: Empty alt text: {img.get('src', 'unknown')}",
                )
        ...
        for link in external_links:
            ...
            if (
                href
                and (href.startswith("http://") or href.startswith("https://"))
                ...
                if not link.get("rel") or "noopener" not in link.get("rel"):
                    self.log_issue(
                        "RECOMMENDATION",
                        "Security",
                        f"{page_name}: External link missing rel='noopener': {href}",
                    )

        # Scripts and loading
        scripts = soup.find_all("script", src=True)
        blocking_scripts = [
            s for s in scripts if not s.get("async") and not s.get("defer")
        ]
        ...
        if len(css_links) > 5:
            self.log_issue(
                "RECOMMENDATION",
                "Performance",
                f"{page_name}: {len(css_links)} CSS files, consider combining",
            )
    ...
    def check_structured_data(self, soup, page_name):
        ...
        if not json_ld:
            self.log_issue(
                "RECOMMENDATION",
                "SEO",
                f"{page_name}: No structured data (JSON-LD) found",
            )
        ...
        if not og_tags:
            self.log_issue(
                "RECOMMENDATION", "SEO", f"{page_name}: No Open Graph tags found"
            )
        ...
        if not twitter_tags:
            self.log_issue(
                "RECOMMENDATION", "SEO", f"{page_name}: No Twitter Card tags found"
            )
    ...
    def check_accessibility_seo(self, soup, page_name):
                    ...
                    if input_id:
                        ...
                        if not label:
                            self.log_issue(
                                "RECOMMENDATION",
                                "Accessibility",
                                f"{page_name}: Input missing associated label: {input_id}",
                            )
        ...
        if not skip_links:
            self.log_issue(
                "RECOMMENDATION",
                "Accessibility",
                f"{page_name}: No skip navigation links found",
            )
    ...
    def check_content_quality(self, soup, page_name):
        ...
        if word_count < 300:
            self.log_issue(
                "RECOMMENDATION",
                "SEO",
                f"{page_name}: Low word count ({word_count}), consider adding more content",
            )
        elif word_count > 2000:
            self.log_issue(
                "RECOMMENDATION",
                "SEO",
                f"{page_name}: High word count ({word_count}), ensure content is well-structured",
            )
        ...
        internal_count = sum(
            ...
            if link.get("href").startswith("/") or self.base_url in link.get("href", "")
        )
        ...
        if internal_count < 3:
            self.log_issue(
                "RECOMMENDATION",
                "SEO",
                f"{page_name}: Few internal links ({internal_count}), consider adding more",
            )

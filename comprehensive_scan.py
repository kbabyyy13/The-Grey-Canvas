...
class ComprehensiveScan:
    ...
    def scan_python_file(self, file_path):
        ...
        try:
            ...
            try:
                ...
                self.log_issue(
                    "code_quality", "high", f"Syntax error in {file_path}: {e}"
                )
    ...
    def check_code_quality(self, content, file_path):
        ...
        for i, line in enumerate(lines, 1):
            if re.search(r"#.*\b(TODO|FIXME|HACK)\b", line, re.IGNORECASE):
                self.log_issue(
                    "code_quality", "info", "TODO/FIXME comment found", file_path, i
                )
    ...
    def scan_template_file(self, file_path):
        ...
        try:
            ...
            for input_elem in inputs:
                if not input_elem.get("aria-label") and not soup.find(
                    "label", {"for": input_elem.get("id")}
                ):
                    self.log_issue(
                        "accessibility", "medium", "Form input missing label", file_path
                    )

            # Check for inline styles (should use CSS)
            elements_with_style = soup.find_all(style=True)
            if (
                len(elements_with_style) > 10
            ):  # Allow some inline styles for color coding
                self.log_issue(
                    "code_quality",
                    "low",
                    f"Many inline styles found ({len(elements_with_style)})",
                    file_path,
                )
            ...
            if "base.html" in file_path:
                ...
                if not viewport:
                    self.log_issue(
                        "seo", "high", "Missing viewport meta tag", file_path
                    )

        except Exception as e:
            self.log_issue(
                "code_quality", "medium", f"Error scanning template {file_path}: {e}"
            )
    ...
    def check_accessibility(self, soup, page_path):
        ...
        if page_path == "/" and not any(
            "skip" in link.text.lower() for link in skip_links
        ):
            self.log_issue(
                "accessibility",
                "medium",
                f"No skip navigation link found on {page_path}",
            )
        ...
        for button in buttons:
            if not button.text.strip() and not button.get("aria-label"):
                self.log_issue(
                    "accessibility",
                    "medium",
                    f"Button without text or aria-label on {page_path}",
                )
    ...
    def generate_report(self):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            if self.accessibility_issues:
                f.write(
                    f"## ♿ Accessibility Issues ({len(self.accessibility_issues)})\n\n"
                )
                for issue in sorted(
                    self.accessibility_issues,
                    key=lambda x: ["info", "low", "medium", "high", "critical"].index(
            ...
            if self.code_quality_issues:
                f.write(
                    f"## 🔧 Code Quality Issues ({len(self.code_quality_issues)})\n\n"
                )
                for issue in sorted(
                    self.code_quality_issues,
                    key=lambda x: ["info", "low", "medium", "high", "critical"].index(
            ...
            if critical_issues > 0:
                ...
                f.write(
                    f"Address all {critical_issues} critical issues immediately before deployment.\n\n"
                )

            if high_issues > 0:
                f.write(f"### ⚠️ High Priority\n")
                f.write(
                    f"Resolve {high_issues} high-priority issues to improve security and functionality.\n\n"
                )

            if medium_issues > 0:
                f.write(f"### 📋 Medium Priority\n")
                f.write(
                    f"Consider addressing {medium_issues} medium-priority issues for optimization.\n\n"
                )

            f.write(f"### 📈 Overall Assessment\n")
            if score >= 90:
                f.write(
                    "**Excellent:** Website is in excellent condition with minimal issues.\n\n"
                )
            elif score >= 80:
                f.write(
                    "**Good:** Website is in good condition with some minor issues to address.\n\n"
                )
            elif score >= 70:
                f.write(
                    "**Fair:** Website has several issues that should be addressed for optimal performance.\n\n"
                )
            elif score >= 60:
                f.write(
